...
Public Class Register
    ...
    Private Sub btns_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btns.Click
        ...
        & "values(@n,@un,@uid,@p,@s,@dob,@em,@pn,@ca,@pa,@c,@st,@zc,@ct)"
            ...
            c = Mid(b, 2, 2)
            Me.Hide()
            Dim a As Integer = MsgBox("Your Diary ID is " & " " & txtus.Text & " " & c & " .Please take note of your ID and keep it Secret", MsgBoxStyle.OkCancel, Title:="Blinks Digital Diary")
            If a = MsgBoxResult.Ok Then
                jpsoft.Parameters.AddWithValue("@zc", txtus.Text & " " & c)
                jpsoft.ExecuteScalar()
                jpsoft.Parameters.Clear()
                Dim aaa As Integer = MsgBox("You have successfully registered as a user", MsgBoxStyle.OkOnly, Title:="Blinks Digital Diary")
                ...
                LoginForm1.Show()
            Else
                Dim aaa As Integer = MsgBox("Your Registration is canceled. Please Start all over", MsgBoxStyle.OkOnly, Title:="Blinks Digital Diary")
                Me.Close()
                LoginForm1.Show()
            End If
